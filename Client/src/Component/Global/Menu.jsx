...
const menuItems = [
  ...
  { id: 6, title: "admin", img: "https://picsum.photos/400/300?random=5" }
]
...
const MenuItem = ({ title, img }) => {
  ...
  return (
    <motion.div
      ...
      style={{ height: "14vh" }}
      animate={{
        backgroundColor: isHovered ? Color.DarkChoc : "rgba(255,255,255,0)",
    ...
    >
      <div className="relative flex items-center justify-center w-full h-full px-10">
        ...
        <motion.h2
          ...
          animate={{ color: isHovered ? Color.ButterYellow :Color.DarkChoc }}
        >
          {title}
        </motion.h2>
      </div>
...
`
  ...
  return (
    <motion.div
      className="fixed top-0 right-0 h-screen flex z-100 pointer-events-none"
      onPointerMove={(e) => {
    ...
    >
      ...
      <motion.div
        className="h-full relative pointer-events-auto shadow-2xl"
      ...
      >
        <motion.div
          className="absolute top-10 right-10 heading text-xl cursor-pointer z-50 overflow-hidden  text-white "
          onClick={() => setMenuStatus(false)}
          style={{color:Color.DarkChoc}}
          whileHover={{color:Color.ButterYellow,textDecoration:'underline'}}
        >
          CLOSE
        </motion.div>
